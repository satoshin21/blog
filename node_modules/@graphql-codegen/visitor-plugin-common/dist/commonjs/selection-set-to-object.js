...
class SelectionSetToObject {
    ...
    _collectInlineFragments(parentType, nodes, types) {
        if (graphql_1.isListType(parentType) || graphql_1.isNonNullType(parentType)) {
            ...
            for (const node of nodes) {
                ...
                const { fields, inlines, spreads } = utils_1.separateSelectionSet(node.selectionSet.selections);
            ...
            const possibleTypes = utils_1.getPossibleTypes(this._schema, parentType);
            for (const node of nodes) {
            ...
            for (const node of nodes) {
                ...
                if (graphql_1.isObjectType(schemaType) && possibleTypes.find(possibleType => possibleType.name === schemaType.name)) {
                    ...
                    const possibleInterfaceTypes = utils_1.getPossibleTypes(this._schema, schemaType);
                    for (const possibleType of possibleTypes) {
    ...
    buildFragmentSpreadsUsage(spreads) {
        ...
        for (const spread of spreads) {
            ...
            if (fragmentSpreadObject) {
                ...
                const possibleTypesForFragment = utils_1.getPossibleTypes(this._schema, schemaType);
    ...
    _buildGroupedSelections() {
        ...
        const grouped = utils_1.getPossibleTypes(this._schema, this._parentSchemaType).reduce((prev, type) => {
    ...
    buildSelectionSetString(parentSchemaType, selectionNodes) {
        ...
        for (const selectionNode of selectionNodes) {
            if (typeof selectionNode === 'string') {
                ...
                if (!selectionNode.selectionSet) {
                    ...
                    const fieldName = utils_1.getFieldNodeNameValue(selectionNode);
                    ...
                    if (!linkFieldNode) {
                        ...
                        utils_1.mergeSelectionSets(linkFieldNode.field.selectionSet, selectionNode.selectionSet);
                    }
                }
            }
        }
        ...
        for (const { field, selectedFieldType } of linkFieldSelectionSets.values()) {
            const realSelectedFieldType = plugin_helpers_1.getBaseType(selectedFieldType);
    ...
    transformFragmentSelectionSetToTypes(fragmentName, fragmentSuffix, declarationBlockConfig) {
        ...
        if (subTypes.length === 1) {
            return new utils_1.DeclarationBlock(declarationBlockConfig)
                .export()
                .asKind('type')
        ...
        return [
            ...subTypes.map(t => new utils_1.DeclarationBlock(declarationBlockConfig)
                .export(this._config.exportFragmentSpreadSubTypes)
                .asKind('type')
                .withName(t.name)
                .withContent(t.content).string),
            new utils_1.DeclarationBlock(declarationBlockConfig)
                .export()
                .asKind('type')
                .withName(this.buildFragmentTypeName(fragmentName, fragmentSuffix))
